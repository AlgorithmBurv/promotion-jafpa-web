...
export default function ManajemenLog() {
  ...
  const [filterTindakan, setFilterTindakan] = useState("Semua");
  ...
  const fetchLogs = async () => {
    setIsLoading(true);
    try {
      // Mengambil log beserta relasi user dan customer/promosi
    ...
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 md:p-8 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Pantau seluruh riwayat pengajuan, revisi, dan persetujuan.</p>
        </div>
        <button onClick={fetchLogs} className="text-sm font-medium text-blue-700 hover:text-blue-900 hover:bg-blue-50 px-3 py-1.5 rounded-md transition-colors border border-transparent hover:border-blue-200">
          Muat Ulang
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 border-y border-slate-100 py-4 bg-slate-50/50 -mx-6 px-6 md:-mx-8 md:px-8">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder="Cari user, customer, atau catatan..." 
            value={searchQuery} 
        ...
        <div className="relative w-full md:w-48">
          ...
          >
            ...
            <option value="Setuju">Setuju (Approval)</option>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead>
            <tr className="bg-slate-50 border-y border-slate-200 text-slate-600 font-semibold text-xs uppercase">
              ...
              <th className="py-3 px-4">Customer & Pengajuan</th>
              <th className="py-3 px-4">Tindakan</th>
              <th className="py-3 px-4 min-w-[250px]">Catatan / Detail</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              <tr key={log.id_log} className="border-b border-slate-100 hover:bg-slate-50 transition-colors align-top">
                <td className="py-3 px-4 text-slate-500">
                  <div className="text-xs font-medium text-slate-800">
                    {new Date(log.waktu_tindakan).toLocaleDateString("id-ID", { day: 'numeric', month: 'short', year: 'numeric' })}
                  </div>
                  <div className="text-[11px] mt-0.5">
                    {new Date(log.waktu_tindakan).toLocaleTimeString("id-ID", { hour: '2-digit', minute:'2-digit' })} WIB
                  </div>
                </td>
                <td className="py-3 px-4">
                  <p className="font-semibold text-slate-800">{log.master_users?.nama_lengkap || 'Unknown'}</p>
                  <p className="text-xs text-slate-500">{log.master_users?.role || '-'}</p>
                </td>
                <td className="py-3 px-4">
                  <p className="font-medium text-slate-800">{log.trx_pengajuan_promosi?.master_customer?.nama_customer || 'Data Terhapus'}</p>
                ...
                <td className="py-3 px-4">
                  <div className="flex items-start gap-2 bg-white border border-slate-200 p-2 rounded-md shadow-sm whitespace-normal text-xs text-slate-600">
                    ...
                    <span className="leading-relaxed">{log.catatan_reviewer || '-'}</span>
                  </div>
                </td>
              </tr>
            ))) : (
      ...
      {processedData.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100 text-sm text-slate-500">
          <span>Menampilkan <span className="font-bold text-slate-700">{indexOfFirstRow + 1}</span> - <span className="font-bold text-slate-700">{Math.min(indexOfLastRow, processedData.length)}</span> dari <span className="font-bold text-slate-700">{processedData.length}</span> log</span>
